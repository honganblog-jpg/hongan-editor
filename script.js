...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const apiKeyInput = document.getElementById('api-key-input'); // [cite: 137]
    const saveKeyBtn = document.getElementById('save-key-btn'); // [cite: 138]

    // Móc API Key đã lưu (Nếu có)
    ...
    // Nén ảnh cho AI soi (maxWidth 1600 để nhìn rõ số VIP)
    const fileToAI = (file) => {
    ...
    generateBtn.addEventListener('click', async () => {
        ...
        try {
            // Bước 1: AI soi Level và VIP (Dùng 1.5 Flash)
            ...
            if (res.ok && data.candidates) {
                ...
                alert("Lỗi 429: Sếp bấm nhanh quá Google chặn rồi. Đợi 1 phút hãy thử lại!");
            }
